...
namespace TapiCallMonitorService
{
    public partial class TapiCallMonitorService : ServiceBase
    {
        ...
        string logFilename = null;       // e.g. "C:\\data\\production\\TapiCallMonitorService\\TapiCallMonitorService.log"
        string connectionString = null;  // e.g. "Server=SERVER;Database=pbx-prd;User Id=service-username;Password=service-password;";
        string versionString = "v0.4";   // appears in txtCallerName column on server start / stop
        ...
        public TapiCallMonitorService()
        {
            ...
            {
                System.Diagnostics.EventLog.CreateEventSource("TapiCallMonitor", "TapiCallMonitorLog");
            }
        ...
        // ******************** TAPI event handlers
        // each of these methods respond to a TAPI event and write that event into the database

        // unsure how to decode call.DeviceSpecificData
        // see http://stackoverflow.com/questions/1003275/how-to-convert-byte-to-string for possible encoding methods
        // also, I sometimes get 'Value cannot be null' exceptions (TapiCall:910)
        // so since I'm not using it going to set it to "" instead
        ...
        private void OnPhoneAdded(object sender, PhoneAddedEventArgs e)
        {
            try
            {
                ...
                addPbxLogRecord("PhoneAdded", line.Id, line.Name, line.DeviceSpecificExtensionID,
                    phone == null ? -1 : phone.Id, phone == null ? "" : phone.Name, -1, -1, -1, "", -1, "", "", "", "", "", "", 0, 0, 0);
            }
            catch (Exception ex)
            {
